...
package org.digimead.tabuddy.desktop.ui.dialog.model.eltemlist
...
object ColumnId extends Loggable {
  ...
  class TEditingSupport(viewer: TableViewer, container: ElementTemplateList) extends EditingSupport(viewer) {
    override protected def getCellEditor(element: AnyRef): CellEditor =
      new IdTextCellEditor(viewer.getTable(), element.asInstanceOf[ElementTemplate.Interface], container)
    override protected def canEdit(element: AnyRef): Boolean =
      !ElementTemplate.predefined.exists(_.id == element.asInstanceOf[ElementTemplate.Interface].id) // exclude predefined
  ...
  class IdTextCellEditor(parent: Composite, item: ElementTemplate.Interface, container: ElementTemplateList) extends TextCellEditor(parent) {
