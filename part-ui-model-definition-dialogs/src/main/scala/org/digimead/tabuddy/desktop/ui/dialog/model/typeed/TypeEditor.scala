...
package org.digimead.tabuddy.desktop.ui.dialog.model.typeed
...
class TypeEditor(val parentShell: Shell, val initial: TypeSchema.Interface, val typeSchemas: List[TypeSchema.Interface], isSchemaActive: Boolean)
  extends org.digimead.tabuddy.desktop.res.dialog.model.TypeEditor(parentShell) with Dialog with Loggable {
  ...
  protected def autoresize() = if (autoResizeLock.tryLock()) try {
    ...
    Main.execNGet {
      if (!getTableViewer.getTable.isDisposed()) {
        ...
        adjustColumnWidth(getTableViewerColumnView(), Default.columnPadding)
        adjustColumnWidth(getTableViewerColumnLabel(), Default.columnPadding)
  ...
  override protected def createDialogArea(parent: Composite): Control = {
    ...
    new ActionContributionItem(TypeEditor.ActionAliasLookup).fill(getCompositeFooter())
