...
package org.digimead.tabuddy.desktop.ui.dialog.model.enumed
...
object ColumnValue extends Loggable {
  ...
  class TEditingSupport(viewer: TableViewer, container: EnumerationEditor) extends EditingSupport(viewer) {
    ...
    override protected def setValue(element: AnyRef, value: AnyRef): Unit = element match {
      case before: EnumerationEditor.Item =>
        val newValue = value.asInstanceOf[String].trim
        if (before.value != newValue)
          container.updateActualConstant(before, before.copy(value = newValue))
      case unknown =>
        log.fatal("Unknown item " + unknown.getClass())
    }
  }
