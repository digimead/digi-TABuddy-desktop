...
import org.digimead.tabuddy.desktop.ui.dialog.model.enumed.EnumerationEditor
...
class JobModifyEnumerationImplementation(
  override val enumeration: Enumeration.Interface[_ <: AnyRef with java.io.Serializable],
  override val enumerationList: Set[Enumeration.Interface[_ <: AnyRef with java.io.Serializable]],
  override val modelID: Symbol) extends JobModifyEnumeration(enumeration, enumerationList, modelID) {
    ...
    val result: Job.Result[Enumeration.Interface[_ <: AnyRef with java.io.Serializable]] = if (canRedo) {
      ...
      Main.execNGet {
        if (Data.getAvailableTypes().isEmpty) {
          ...
          val dialog = new EnumerationEditor(Window.currentShell(), enumeration, enumerationList.toList)
