...
class ElementTemplateEditor(val parentShell: Shell,
  ...
  extends org.digimead.tabuddy.desktop.res.dialog.model.ElementTemplateEditor(parentShell) with Dialog with Loggable {
  ...
  def getModifiedTemplate(): ElementTemplate.Interface = {
      ...
      case (group, properties) =>
        properties.map(property =>
          ElementTemplateEditor.Item(property.id.name, None, // id column
            ...
            property.enumeration.flatMap(Data.enumerations.get).asInstanceOf[Option[Enumeration.Interface[AnyRef with java.io.Serializable]]],
