...
object ColumnName extends Loggable {
  class TLabelProvider extends CellLabelProvider {
    ...
    override def update(cell: ViewerCell) = cell.getElement() match {
      case item: TypeSchema.Interface =>
  ...
  class TEditingSupport(viewer: TableViewer, container: TypeList) extends EditingSupport(viewer) {
    override protected def getCellEditor(element: AnyRef): CellEditor =
      new NameTextCellEditor(viewer.getTable(), element.asInstanceOf[TypeSchema.Interface], container)
    ...
    override protected def setValue(element: AnyRef, value: AnyRef): Unit = element match {
      case before: TypeSchema.Interface if before.name != value.asInstanceOf[String] =>
      ...
      case before: TypeSchema.Interface =>
      case unknown =>
        log.fatal("Unknown item " + unknown.getClass())
    }
  }
  class NameTextCellEditor(parent: Composite, schema: TypeSchema.Interface, container: TypeList) extends TextCellEditor(parent) {
