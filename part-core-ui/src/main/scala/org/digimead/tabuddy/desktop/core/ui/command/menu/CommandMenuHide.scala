 ...
 * Hide menu entry.
 */
object CommandMenuHide {
  import Command.parser._
  /** Akka execution context. */
  implicit lazy val ec = App.system.dispatcher
  /** Command description. */
  implicit lazy val descriptor = Command.Descriptor(UUID.randomUUID())(Messages.menu_hide_text,
    Messages.menu_hideDescriptionShort_text, Messages.menu_hideDescriptionLong_text,
    (activeContext, parserContext, parserResult) ⇒ Future {
      val id = parserResult.asInstanceOf[String]
      SmartMenuManager.hide(id)
      "Hide " + id
    })
  /** Command parser. */
