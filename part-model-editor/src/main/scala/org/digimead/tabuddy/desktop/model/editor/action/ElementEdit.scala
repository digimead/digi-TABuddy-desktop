...
object ElementEdit extends Handler.Singleton with Loggable {
  /** ElementEdit actor path. */
  lazy val actorPath = App.system / Core.id / Editor.id / ElementToolBar.id / id
  /** Command id for the current handler. */
  val commandId = classOf[ElementEdit].getName
  /** Handler actor reference configuration object. */
  val props = DI.props

  class Behavoiur extends Handler.Behaviour(ElementEdit) with Loggable
  /**
   * Dependency injection routines
   */
  private object DI extends DependencyInjection.PersistentInjectable {
    /** ElementEdit actor reference configuration object. */
    lazy val props = injectOptional[Props]("command.ElementEdit") getOrElse Props[Behavoiur]
  }
}
*/
