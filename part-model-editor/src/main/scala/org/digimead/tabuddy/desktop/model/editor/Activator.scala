...
class Activator extends BundleActivator with XLoggable {
  ...
  def start(context: BundleContext) = Activator.startStopLock.synchronized {
    ...
    val f = Future {
      Activator.startStopLock.synchronized {
        App.watch(Activator).once.makeBeforeStop('model_editor_Activator__WaitingForContext) {
    ...
    App.watch(core.Activator).once.makeBeforeStop('model_editor_Activator__LockCore) {
  ...
  override def toString = "model.editor.Activator"
}
...
object Activator extends Disposable.Manager with XLoggable {
  ...
  override def toString = "model.editor.Activator[Singleton]"
