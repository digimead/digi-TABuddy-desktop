...
package org.digimead.tabuddy.desktop.model.editor.action
...
class ElementNew extends JFaceAction("ElementNew") with Loggable {
...
/*
class ElementNew extends Handler(ElementNew) with Loggable {
  @log
...
object ElementNew extends Handler.Singleton with Loggable {
  /** ElementNew actor path. */
  lazy val actorPath = App.system / Core.id / Editor.id / ElementToolBar.id / id
  /** Command id for the current handler. */
  val commandId = classOf[ElementNew].getName
  /** Handler actor reference configuration object. */
  val props = DI.props

  class Behavoiur extends Handler.Behaviour(ElementNew) with Loggable
  /**
   * Dependency injection routines
   */
  private object DI extends DependencyInjection.PersistentInjectable {
    /** ElementNew actor reference configuration object. */
    lazy val props = injectOptional[Props]("command.ElementNew") getOrElse Props[Behavoiur]
  }
}
*/

object ElementNew extends Loggable {
  /** Singleton identificator. */
  val id = getClass.getSimpleName().dropRight(1)
  /** ElementNew action. */
  @volatile protected var action: Option[ElementNew] = None

  /** Returns ElementNew action. */
  def apply(): ElementNew = action.getOrElse {
    val ElementNewAction = App.execNGet { new ElementNew }
    action = Some(ElementNewAction)
    ElementNewAction
  }
  /** ElementNew actor reference configuration object. */
  def props = DI.props
  ...
  private object DI extends DependencyInjection.PersistentInjectable {
    /** ElementNew actor reference configuration object. */
    lazy val props = injectOptional[Props]("Editor.Action.ElementNew") getOrElse Props[ElementNew.Actor]
  }
}
