...
class ToggleSystem extends JFaceAction(Messages.systemElements_text, IAction.AS_CHECK_BOX) with Loggable {
  ...
  @Inject @Optional
  def onStateOfToggleIdentificatorChanged(@Active @Named(Editor.Id.stateOfToggleSystem) checked: java.lang.Boolean) =
    Option(checked) foreach { checked => App.exec { if (checked != isChecked()) setChecked(checked) } }
  /** Runs this action. */
  @log
  override def run() = for {
    v <- App.getActiveView if v.getChildren().headOption.map(_.isInstanceOf[org.digimead.tabuddy.desktop.model.editor.view.editor.View]).getOrElse(false)
  } v.getContext.set(Editor.Id.stateOfToggleSystem, isChecked(): java.lang.Boolean)
...
object ToggleSystem extends Loggable {
  ...
  /** ToggleSystem action. */
  @volatile protected var action: Option[ToggleSystem] = None

  /** Returns ToggleSystem action. */
  def apply(): ToggleSystem = action.getOrElse {
    val ToggleSystemAction = App.execNGet { new ToggleSystem }
    action = Some(ToggleSystemAction)
    ToggleSystemAction
  }
  /** ToggleSystem actor reference configuration object. */
  def props = DI.props
  ...
  private object DI extends DependencyInjection.PersistentInjectable {
    /** ToggleSystem actor reference configuration object. */
    lazy val props = injectOptional[Props]("Editor.Action.ToggleSystem") getOrElse Props[ToggleSystem.Actor]
  }
}
