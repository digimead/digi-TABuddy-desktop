...
class ModelDefinition extends akka.actor.Actor with Loggable {
  ...
  def receive = {
    ...
    case message @ App.Message.Inconsistent(element, _) if element != ModelDefinition && App.bundle(element.getClass()) == thisBundle => App.traceMessage(message) {
    ...
    case message @ App.Message.Consistent(element, _) if element != ModelDefinition && App.bundle(element.getClass()) == thisBundle => App.traceMessage(message) {
      inconsistentSet = inconsistentSet - element
      if (inconsistentSet.isEmpty) {
  ...
  protected def onGUIValid() = initializationLock.synchronized {
    App.afterStart("Desktop Model Definition", Timeout.normal.toMillis, Logic.getClass()) {
      ...
      //Actions.configure
      App.markAsStarted(ModelDefinition.getClass)
    }
  }
  ...
  protected def onGUIInvalid() = initializationLock.synchronized {
    val context = thisBundle.getBundleContext()
    App.markAsStopped(ModelDefinition.getClass())
    ...
    App.publish(App.Message.Consistent(ModelDefinition, self))
  }
}
