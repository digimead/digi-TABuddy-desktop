...
package org.digimead.tabuddy.desktop.modeldef.dialog.typelist
...
object ColumnName extends Loggable {
  ...
  class TEditingSupport(viewer: TableViewer, container: TypeList) extends EditingSupport(viewer) {
    override protected def getCellEditor(element: AnyRef): CellEditor =
      new NameTextCellEditor(viewer.getTable(), element.asInstanceOf[TypeSchema], container)
  ...
  class NameTextCellEditor(parent: Composite, schema: TypeSchema, container: TypeList) extends TextCellEditor(parent) {
