...
package org.digimead.tabuddy.desktop.ui.dialog.view.filtered
...
import org.digimead.tabuddy.desktop.payload.view.{ Filter => FilterPayload }
...
class FilterEditor(val parentShell: Shell, val filter: FilterPayload, val filterList: List[FilterPayload])
  extends org.digimead.tabuddy.desktop.res.dialog.view.FilterEditor(parentShell) with Dialog with Loggable {
  /** The actual content */
  ...
  protected val selectedRule = WritableValue[FilterPayload.Rule]
  ...
  def getModifiedFilter(): FilterPayload = {
    val name = nameField.value.trim
    val description = descriptionField.value.trim
    FilterPayload(filter.id, name, description, availabilityField.value, mutable.LinkedHashSet(actual: _*))
  }
  ...
  protected def autoresize() = if (autoResizeLock.tryLock()) try {
    ...
    Main.execNGet {
      if (!getTableViewerFilters.getTable.isDisposed() && !getTableViewerProperties.getTable.isDisposed()) {
        ...
        adjustColumnWidth(getTableViewerColumnInversion(), Default.columnPadding)
        adjustColumnWidth(getTableViewerColumnFilter(), Default.columnPadding)
  ...
  override protected def createDialogArea(parent: Composite): Control = {
    ...
    new ActionContributionItem(FilterEditor.ActionAdd).fill(getCompositeBody())
    new ActionContributionItem(FilterEditor.ActionRemove).fill(getCompositeBody())
    FilterEditor.ActionAdd.setEnabled(false)
    FilterEditor.ActionRemove.setEnabled(false)
  ...
  protected def initTableViewerProperties() {
    ...
    menuMgr.addMenuListener(new IMenuListener() {
      override def menuAboutToShow(manager: IMenuManager) {
        manager.add(FilterEditor.ActionAutoResize)
      }
    })
    ...
    viewer.addSelectionChangedListener(new ISelectionChangedListener() {
      override def selectionChanged(event: SelectionChangedEvent) = event.getSelection() match {
        case selection: IStructuredSelection if !selection.isEmpty() =>
          FilterEditor.ActionAdd.setEnabled(true)
        case selection =>
          FilterEditor.ActionAdd.setEnabled(false)
      }
    })
  ...
  protected def initTableViewerSortings() {
    ...
    viewer.addSelectionChangedListener(new ISelectionChangedListener() {
      override def selectionChanged(event: SelectionChangedEvent) = event.getSelection() match {
        case selection: IStructuredSelection if !selection.isEmpty() =>
          FilterEditor.ActionRemove.setEnabled(true)
        case selection =>
          FilterEditor.ActionRemove.setEnabled(false)
      }
    })
    // Add the filter
    Main.bindingContext.bindValue(WidgetProperties.text(SWT.Modify).observeDelayed(50, getTextFilterFilters()), filterFilters)
    val filter = new AtomicReference(".*".r.pattern)
    val filterListener = new BaseElement.RegexFilterListener(filter) {
      override def handleChange(event: ChangeEvent) {
        super.handleChange(event)
    ...
    Main.bindingContext.bindValue(ViewersObservables.observeSingleSelection(viewer), selectedRule)
  }
  /** On dialog active */
  ...
  protected[filtered] def updateActualRule(before: FilterPayload.Rule, after: FilterPayload.Rule) {
...
object FilterEditor extends Loggable {
  ...
  /** Apply a f(x) to the selected view if any */
  def rule[T](f: (FilterEditor, FilterPayload.Rule) => T): Option[T] =
    dialog.flatMap(d => Option(d.selectedRule.value).map(f(d, _)))

  abstract class AbstractFilterEditor(parentShell: Shell) extends org.digimead.tabuddy.desktop.res.dialog.view.FilterEditor(parentShell)
  ...
  class FilterComparator(initialColumn: Int, initialDirection: Boolean = Default.sortingDirection) extends ViewerComparator {
    ...
    override def compare(viewer: Viewer, e1: Object, e2: Object): Int = {
      e1 match {
        case entity1: FilterPayload.Rule =>
          val entity2 = e2.asInstanceOf[FilterPayload.Rule]
          val rc = column match {
  ...
  class Filter(filter: AtomicReference[Pattern]) extends ViewerFilter {
    override def select(viewer: Viewer, parentElement: AnyRef, element: AnyRef): Boolean = {
      val pattern = filter.get
      val item = element.asInstanceOf[FilterPayload.Rule]
