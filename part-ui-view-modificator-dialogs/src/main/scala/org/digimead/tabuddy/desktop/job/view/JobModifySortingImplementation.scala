...
import org.digimead.tabuddy.desktop.ui.dialog.view.sorted.SortingEditor
import org.digimead.tabuddy.model.Model
import org.digimead.tabuddy.model.Model.model2implementation
import org.eclipse.core.runtime.IAdaptable
import org.eclipse.core.runtime.IProgressMonitor

class JobModifySortingImplementation(
  override val sorting: Sorting,
  override val sortingList: Set[Sorting],
  override val modelID: Symbol)
  extends JobModifySorting(sorting, sortingList, modelID) with Loggable {
  ...
  protected def redo(monitor: IProgressMonitor, info: IAdaptable): Job.Result[Sorting] = {
    assert(Model.eId == modelID, "An unexpected model %s, expect %s".format(Model.eId, modelID))
    // process the job
    val result: Job.Result[Sorting] = if (canRedo) {
      ...
      Main.execNGet {
        val dialog = new SortingEditor(Window.currentShell(), sorting, sortingList.toList)
        Window.currentShell.withValue(Some(dialog.getShell)) {
          dialog.open() == org.eclipse.jface.window.Window.OK
        } match {
          case true => Job.Result.OK(Option(dialog.getModifiedSorting()))
  ...
  protected def undo(monitor: IProgressMonitor, info: IAdaptable): Job.Result[Sorting] = {
