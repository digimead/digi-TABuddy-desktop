...
import org.digimead.tabuddy.desktop.ui.dialog.view.filterlist.FilterList
import org.digimead.tabuddy.model.Model
import org.digimead.tabuddy.model.Model.model2implementation
import org.eclipse.core.runtime.IAdaptable
import org.eclipse.core.runtime.IProgressMonitor

class JobModifyFilterListImplementation(
  override val filterList: Set[Filter],
  override val modelID: Symbol) extends JobModifyFilterList(filterList, modelID) with Loggable {
  ...
  protected def redo(monitor: IProgressMonitor, info: IAdaptable): Job.Result[Set[Filter]] = {
    assert(Model.eId == modelID, "An unexpected model %s, expect %s".format(Model.eId, modelID))
    // process the job
    val result: Job.Result[Set[Filter]] = if (canRedo) {
      ...
      Main.execNGet {
        val dialog = new FilterList(Window.currentShell(), filterList.toList)
        Window.currentShell.withValue(Some(dialog.getShell)) {
          dialog.open() == org.eclipse.jface.window.Window.OK
        } match {
          case true => Job.Result.OK(Some(dialog.getModifiedFilters()))
  ...
  protected def undo(monitor: IProgressMonitor, info: IAdaptable): Job.Result[Set[Filter]] = {
