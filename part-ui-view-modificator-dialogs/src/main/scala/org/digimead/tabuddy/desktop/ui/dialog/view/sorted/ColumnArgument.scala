  ...
  class TLabelProvider extends CellLabelProvider {
    ...
    override def update(cell: ViewerCell) = cell.getElement() match {
      case item: Sorting.Definition =>
        cell.setText(Comparator.map.get(item.comparator).flatMap(c => c.stringToArgument(item.argument).map(c.argumentToText)).getOrElse(item.argument))
      case unknown =>
    ...
    override def getToolTipText(element: Object): String = element match {
      case item: Sorting.Definition =>
        Comparator.map.get(item.comparator).map(c => "comparator: " + c.description).getOrElse(null)
      case unknown =>
        log.fatal("Unknown item " + unknown.getClass())
        null
    }
