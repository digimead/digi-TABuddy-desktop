...
package org.digimead.tabuddy.desktop.view.modification.dialog.filterlist
...
object ColumnName extends Loggable {
  ...
  class TEditingSupport(viewer: TableViewer, container: FilterList) extends EditingSupport(viewer) {
    override protected def getCellEditor(element: AnyRef): CellEditor =
      new NameTextCellEditor(viewer.getTable(), element.asInstanceOf[Filter], container)
    ...
    override protected def setValue(element: AnyRef, value: AnyRef): Unit = element match {
      case before: Filter if before.name != value.asInstanceOf[String] ⇒
        val name = value.asInstanceOf[String].trim
      //        if (!container.actual.exists(_.name == name))
      //          container.updateActualSort(before, before.copy(name = name))
      case before: Filter ⇒
      case unknown ⇒
        log.fatal("Unknown item " + unknown.getClass())
    }
  }
  class NameTextCellEditor(parent: Composite, filter: Filter, container: FilterList) extends TextCellEditor(parent) {
    ...
    override def createControl(parent: Composite): Control = {
      ...
      WidgetProperties.text(SWT.Modify).observe(text).addChangeListener(new IChangeListener() {
        override def handleChange(event: ChangeEvent) = {
          ...
          else if (container.actual.exists(_.name == newName) && newName != filter.name)
