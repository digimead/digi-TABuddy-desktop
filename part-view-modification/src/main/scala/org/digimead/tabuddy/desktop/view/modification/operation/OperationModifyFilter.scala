...
class OperationModifyFilter extends logic.operation.view.OperationModifyFilter with XLoggable {
   ...
   */
  def apply(graph: Graph[_ <: Model.Like], filter: Filter, filterList: Set[Filter]): Filter = {
    log.info(s"Modify ${filter} for ${graph}.")
    dialog(graph, filter, filterList) match {
      case Operation.Result.OK(Some(filter), _) ⇒ filter
      case _ ⇒ filter
    }
  }
  /**
   ...
   */
  def operation(graph: Graph[_ <: Model.Like], filter: Filter, filterList: Set[Filter]) =
    new Implemetation(graph, filter, filterList)

  protected def dialog(graph: Graph[_ <: Model.Like], filter: Filter, filterList: Set[Filter]): Operation.Result[Filter] = {
    val marker = GraphMarker(graph)
    val exchanger = new Exchanger[Operation.Result[Filter]]()
        ...
        GraphMarker.shell(marker) match {
          case Some((context, shell)) ⇒
            ...
            marker.safeRead { state ⇒
              val dialogContext = context.createChild("FilterEditorDialog")
              ...
              dialog.openOrFocus { result ⇒
                ...
                if (result == org.eclipse.jface.window.Window.OK)
                  exchanger.exchange(Operation.Result.OK(Some(dialog.getModifiedFilter())))
                else
  ...
  class Implemetation(graph: Graph[_ <: Model.Like], filter: Filter, filterList: Set[Filter])
    ...
    protected def execute(monitor: IProgressMonitor, info: IAdaptable): Operation.Result[Filter] =
      try dialog(graph, filter, filterList)
      catch {
