...
class ContributionSelectFilter(val window: WeakReference[AppWindow]) extends ControlContribution(ContributionSelectFilter.id) with ContributionSelectBase[Filter] with Loggable {
  /** Context value key. */
...
//        updateContextValue(Some(Default.ViewModification.filter))
...
//    App.exec { reloadItems(Option(id).flatMap(Data.viewDefinitions.get) getOrElse { Default.ViewModification.view }) }
...
//      available.toArray
//    else
//      Default.ViewModification.filter +: view.filters.flatMap(id =>
//        available.find(_.id == id && id != Default.ViewModification.filter.id)).toArray
...
//      return
//    if (selection == Some(Default.ViewModification.filter) && value.isEmpty)
//      return
...
//        case _ =>
//          log.debug(s"Set UI value to ${Default.ViewModification.filter.id}.")
//          comboViewer.setSelection(new StructuredSelection(Default.ViewModification.filter), true)
//      }
  }
}
