...
object ColumnName extends Loggable {
  ...
  class TEditingSupport(viewer: TableViewer, container: SortingList) extends EditingSupport(viewer) {
    override protected def getCellEditor(element: AnyRef): CellEditor =
      new NameTextCellEditor(viewer.getTable(), element.asInstanceOf[Sorting], container)
    ...
    override protected def setValue(element: AnyRef, value: AnyRef): Unit = element match {
      case before: Sorting if before.name != value.asInstanceOf[String] ⇒
        val name = value.asInstanceOf[String].trim
        if (!container.actual.exists(_.name == name))
          container.updateActualSorting(before, before.copy(name = name))
      case before: Sorting ⇒
      case unknown ⇒
        log.fatal("Unknown item " + unknown.getClass())
    }
  }
  class NameTextCellEditor(parent: Composite, sort: Sorting, container: SortingList) extends TextCellEditor(parent) {
    ...
    override def createControl(parent: Composite): Control = {
      ...
      WidgetProperties.text(SWT.Modify).observe(text).addChangeListener(new IChangeListener() {
        override def handleChange(event: ChangeEvent) = {
          ...
          else if (container.actual.exists(_.name == newName) && newName != sort.name)
