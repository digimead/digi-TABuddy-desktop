...
import org.digimead.tabuddy.desktop.ui.dialog.view.filtered.FilterEditor
import org.digimead.tabuddy.model.Model
import org.digimead.tabuddy.model.Model.model2implementation
import org.eclipse.core.runtime.IAdaptable
import org.eclipse.core.runtime.IProgressMonitor

class JobModifyFilterImplementation(
  override val filter: Filter,
  override val filterList: Set[Filter],
  override val modelID: Symbol)
  extends JobModifyFilter(filter, filterList, modelID) with Loggable {
  ...
  protected def redo(monitor: IProgressMonitor, info: IAdaptable): Job.Result[Filter] = {
    assert(Model.eId == modelID, "An unexpected model %s, expect %s".format(Model.eId, modelID))
    // process the job
    val result: Job.Result[Filter] = if (canRedo) {
      ...
      Main.execNGet {
        val dialog = new FilterEditor(Window.currentShell(), filter, filterList.toList)
        Window.currentShell.withValue(Some(dialog.getShell)) {
          dialog.open() == org.eclipse.jface.window.Window.OK
        } match {
          case true => Job.Result.OK(Option(dialog.getModifiedFilter()))
  ...
  protected def undo(monitor: IProgressMonitor, info: IAdaptable): Job.Result[Filter] = {
