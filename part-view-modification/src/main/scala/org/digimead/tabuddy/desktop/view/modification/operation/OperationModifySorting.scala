...
class OperationModifySorting extends logic.operation.view.OperationModifySorting with XLoggable {
  /**
   * Modify sorting.
   *
        ...
        GraphMarker.shell(marker) match {
          case Some((context, shell)) ⇒
            ...
            marker.safeRead { state ⇒
              ...
              dialogContext.set(classOf[List[Sorting]], sortingList.toList)
  ...
  class Implemetation(graph: Graph[_ <: Model.Like], sorting: Sorting, sortingList: Set[Sorting])
    extends logic.operation.view.OperationModifySorting.Abstract(graph, sorting, sortingList) with XLoggable {
