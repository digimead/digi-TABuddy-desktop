...
import org.digimead.tabuddy.desktop.ui.dialog.view.viewed.ViewEditor
import org.digimead.tabuddy.model.Model
import org.digimead.tabuddy.model.Model.model2implementation
import org.eclipse.core.runtime.IAdaptable
import org.eclipse.core.runtime.IProgressMonitor

class JobModifyViewImplementation(
  override val view: View,
  override val viewList: Set[View],
  override val modelID: Symbol) extends JobModifyView(view, viewList, modelID) with Loggable {
  ...
  protected def redo(monitor: IProgressMonitor, info: IAdaptable): Job.Result[View] = {
    assert(Model.eId == modelID, "An unexpected model %s, expect %s".format(Model.eId, modelID))
    // process the job
    val result: Job.Result[View] = if (canRedo) {
      ...
      Main.execNGet {
        val dialog = new ViewEditor(Window.currentShell(), view, viewList.toList)
  ...
  protected def undo(monitor: IProgressMonitor, info: IAdaptable): Job.Result[View] = {
