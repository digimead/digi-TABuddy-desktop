...
/**
 * Expand new elements.
 * checked - expand is enabled
 * unchecked - expand is disabled
 * by default - enabled
 */
class ActionToggleExpand @Inject extends Action(Messages.expandNew_text, IAction.AS_CHECK_BOX) with XLoggable {
  setId(ActionToggleExpand.id)
  setChecked(false)
  setEnabled(false)
  ...
  def apply() = contentContext match {
    case Some(contentContext) ⇒
      Option(contentContext.getLocal(Logic.Id.stateOfToggleExpand)) match {
        case Some(java.lang.Boolean.TRUE) ⇒
          if (!isChecked())
            contentContext.set(Logic.Id.stateOfToggleExpand, java.lang.Boolean.FALSE)
        case Some(java.lang.Boolean.FALSE) ⇒
          if (isChecked())
            contentContext.set(Logic.Id.stateOfToggleExpand, java.lang.Boolean.TRUE)
        case _ ⇒
          contentContext.set(Logic.Id.stateOfToggleExpand, isChecked(): java.lang.Boolean)
      }
    case None ⇒
  ...
  @Inject @Optional @log
  protected def onStateChanged(@Active @Named(Logic.Id.stateOfToggleExpand) state: java.lang.Boolean) = App.exec {
    if (isChecked() != state)
      setChecked(state)
  }
  ...
  protected def onViewChanged(@Active vComposite: VComposite): Unit = App.exec {
    if (vComposite.factory().features.contains(Logic.Feature.viewToggleExpand)) {
      if (!isEnabled())
        setEnabled(true)
        ...
        Option(contentContext.getLocal(Logic.Id.stateOfToggleExpand)) match {
          ...
          case Some(java.lang.Boolean.FALSE) ⇒
            if (!isChecked())
              setChecked(false)
          case _ ⇒
            contentContext.set(Logic.Id.stateOfToggleExpand, default)
        }
        contentContext
      }
    } else {
  ...
  object PropertyChangeListener extends IPropertyChangeListener {
    def propertyChange(e: PropertyChangeEvent) = contentContext.foreach { ctx ⇒
      ...
      if (ctx.getLocal(Logic.Id.stateOfToggleExpand) != value)
        ctx.set(Logic.Id.stateOfToggleExpand, value)
    }
  }
}

object ActionToggleExpand {
  val id = bundleId + "#ToggleExpandNew"
}
