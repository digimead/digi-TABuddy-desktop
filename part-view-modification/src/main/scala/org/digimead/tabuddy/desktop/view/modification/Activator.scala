...
class Activator extends BundleActivator with XLoggable {
  ...
  def start(context: BundleContext) = Activator.startStopLock.synchronized {
    ...
    log.debug("Start TABuddy Desktop ViewModification component.")
    ...
    val f = Future {
      Activator.startStopLock.synchronized {
        App.watch(Activator).once.makeBeforeStop('view_modification_Activator__WaitingForContext) {
    ...
    App.watch(core.Activator).once.makeBeforeStop('view_modification_Activator__LockCore) {
  ...
  def stop(context: BundleContext) = Activator.startStopLock.synchronized {
    log.debug("Stop TABuddy Desktop ViewModification component.")
    ViewModification ! App.Message.Inconsistent(ViewModification, None)
    App.watch(Activator) off {}
    App.watch(ViewModification).waitForStop(Timeout.long)
    try {
      // Stop component actors.
      val inbox = Inbox.create(App.system)
      inbox.watch(ViewModification)
      ViewModification ! PoisonPill
      if (inbox.receive(Timeout.long).isInstanceOf[Terminated])
        log.debug("ViewModification actors hierarchy is terminated.")
      else
        log.fatal("Unable to shutdown ViewModification actors hierarchy.")
    } catch {
