...
class OperationModifyViewList extends logic.operation.view.OperationModifyViewList with Loggable {
   ...
   */
  def apply(graph: Graph[_ <: Model.Like], viewList: Set[View]): Set[View] = {
    log.info(s"Modify the view list of ${graph}.")
    dialog(graph, viewList) match {
      case Operation.Result.OK(Some(viewList), _) ⇒ viewList
      case _ ⇒ viewList
    }
  }
  /**
   ...
   */
  def operation(graph: Graph[_ <: Model.Like], viewList: Set[View]) =
    new Implemetation(graph, viewList)

  protected def dialog(graph: Graph[_ <: Model.Like], viewList: Set[View]): Operation.Result[Set[View]] = {
        ...
        GraphMarker.shell(marker) match {
          case Some((context, shell)) ⇒
            ...
            marker.safeRead { state ⇒
              val dialogContext = context.createChild("ViewListDialog")
              ...
              val dialog = ContextInjectionFactory.make(classOf[ViewList], dialogContext)
