...
import org.digimead.tabuddy.desktop.view.modification.dialog.viewlist.ViewList
...
class OperationModifyViewList extends api.OperationModifyViewList with Loggable {
  /**
   * Modify view list.
   *
  ...
  def apply(viewList: Set[View], modelId: Symbol): Set[View] = {
    log.info(s"Modify view list for model ${modelId}.")
    ...
    App.getActiveShell() match {
      ...
      case None =>
        throw new IllegalStateException("Unable to create 'modify view list' dialog without parent shell.")
    }
   ...
   */
  def operation(viewList: Set[View], modelId: Symbol): Operation[Set[View]] =
    new Implemetation(viewList, modelId)

  class Implemetation(viewList: Set[View], modelId: Symbol)
    extends logic.operation.view.OperationModifyViewList.Abstract(viewList, modelId) with Loggable {
    ...
    protected def execute(monitor: IProgressMonitor, info: IAdaptable): Operation.Result[Set[View]] = {
      try {
        Operation.Result.OK(Option(OperationModifyViewList.this(viewList, modelId)))
      } catch {
        case e: CancellationException =>
          Operation.Result.Cancel()
      }
    }
    protected def redo(monitor: IProgressMonitor, info: IAdaptable): Operation.Result[Set[View]] =
      throw new UnsupportedOperationException
    protected def undo(monitor: IProgressMonitor, info: IAdaptable): Operation.Result[Set[View]] =
      throw new UnsupportedOperationException
  }
}
