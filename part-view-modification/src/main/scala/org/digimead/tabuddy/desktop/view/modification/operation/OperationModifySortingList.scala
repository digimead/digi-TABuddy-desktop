...
class OperationModifySortingList extends logic.operation.view.OperationModifySortingList with Loggable {
   ...
   */
  def apply(graph: Graph[_ <: Model.Like], sortingList: Set[Sorting]): Set[Sorting] = {
    log.info(s"Modify the sorting list of ${graph}.")
    dialog(graph, sortingList) match {
      case Operation.Result.OK(Some(sortingList), _) ⇒ sortingList
      case _ ⇒ sortingList
    }
  }
  /**
   ...
   */
  def operation(graph: Graph[_ <: Model.Like], sortingList: Set[Sorting]) =
    new Implemetation(graph, sortingList)

  protected def dialog(graph: Graph[_ <: Model.Like], sortingList: Set[Sorting]): Operation.Result[Set[Sorting]] = {
        ...
        GraphMarker.shell(marker) match {
          case Some((context, shell)) ⇒
            ...
            marker.safeRead { state ⇒
              val dialogContext = context.createChild("SortingListDialog")
              ...
              val dialog = ContextInjectionFactory.make(classOf[SortingList], dialogContext)
              dialog.openOrFocus { result ⇒
                ...
                if (result == org.eclipse.jface.window.Window.OK)
                  exchanger.exchange(Operation.Result.OK(Some(dialog.getModifiedSortings())))
                else
