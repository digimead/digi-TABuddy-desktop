...
import org.digimead.tabuddy.desktop.ui.dialog.view.viewlist.ViewList
import org.digimead.tabuddy.model.Model
import org.digimead.tabuddy.model.Model.model2implementation
import org.eclipse.core.runtime.IAdaptable
import org.eclipse.core.runtime.IProgressMonitor

class JobModifyViewListImplementation(
  override val views: Set[View],
  override val modelID: Symbol) extends JobModifyViewList(views, modelID) with Loggable {
  ...
  protected def redo(monitor: IProgressMonitor, info: IAdaptable): Job.Result[Set[View]] = {
    assert(Model.eId == modelID, "An unexpected model %s, expect %s".format(Model.eId, modelID))
    // process the job
    val result: Job.Result[Set[View]] = if (canRedo) {
      ...
      Main.execNGet {
        val dialog = new ViewList(Window.currentShell(), views.toList)
  ...
  protected def undo(monitor: IProgressMonitor, info: IAdaptable): Job.Result[Set[View]] = {
