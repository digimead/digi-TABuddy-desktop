...
/**
 * Modify a view definition list.
 */
object CommandModifyViewList extends Loggable {
  import Command.parser._
  /** Akka execution context. */
  implicit lazy val ec = App.system.dispatcher
  /** Command description. */
  implicit lazy val descriptor = Command.Descriptor(UUID.randomUUID())(Messages.modifyViewDefinitionList_text,
    Messages.modifyViewDefinitionListDescriptionShort_text, Messages.modifyViewDefinitionListDescriptionLong_text,
      ...
      parserResult match {
        case marker: GraphMarker ⇒
          ...
          marker.safeRead { state ⇒
            App.exec {
              OperationModifyViewList(state.graph, state.payload.viewDefinitions.values.toSet).foreach { operation ⇒
  ...
  object HintContainer extends Command.Hint.Container {
    ...
    def apply(arg: String): Seq[Command.Hint] =
      (Core.context.context +: App.contextChildren(Core.context)).map {
        ...
        case Some(name) ⇒
          Some(Command.Hint(name, Some(s"Modify view definition list for " +
