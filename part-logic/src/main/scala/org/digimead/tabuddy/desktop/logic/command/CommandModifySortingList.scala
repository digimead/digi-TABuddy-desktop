 ...
 */
object CommandModifySortingList extends XLoggable {
  import Command.parser._
  /** Akka execution context. */
  implicit lazy val ec = App.system.dispatcher
  /** Command description. */
  implicit lazy val descriptor = Command.Descriptor(UUID.randomUUID())(Messages.modifyViewSortingList_text,
    Messages.modifyViewSortingListDescriptionShort_text, Messages.modifyViewSortingListDescriptionLong_text,
  ...
  object HintContainer extends Command.Hint.Container {
    ...
    def apply(arg: String): Seq[Command.Hint] =
      (Core.context.context +: App.contextChildren(Core.context)).map {
        ...
        case Some(name) ⇒
          Some(Command.Hint(name, Some(s"Modify view sorting list for " +
