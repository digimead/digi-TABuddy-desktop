 ...
 */
object JobModifyElementTemplate extends Loggable {
  @log
  def apply(
  ...
  abstract class Abstract(
    ...
    val modelID: Symbol)
    extends Job[ElementTemplate](s"Edit $template for model $modelID") with api.JobModifyElementTemplate
  /**
   * Dependency injection routines.
   */
  private object DI extends DependencyInjection.PersistentInjectable {
    // Element[_ <: Stash] == Element.Generic, avoid 'erroneous or inaccessible type' error
    lazy val jobFactory = injectOptional[(ElementTemplate, Set[ElementTemplate], Symbol) => api.JobModifyElementTemplate]
  }
}
