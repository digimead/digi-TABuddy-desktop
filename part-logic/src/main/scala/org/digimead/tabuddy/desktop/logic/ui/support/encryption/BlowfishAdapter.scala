...
/**
 * Adapter between logic.payload.marker.serialization.encryption.Blowfish and UI
 */
class BlowfishAdapter extends EncryptionAdapter {
  ...
  def composite(parent: Composite, default: Option[Encryption.Parameters]) =
    default match {
      case Some(p: Blowfish.Parameters) ⇒
        Option(new BlowfishAdapter.BlowfishComposite(parent, SWT.NONE, Option(p)))
      case Some(other) ⇒
        throw new IllegalArgumentException(s"Expect Blowfish.Parameters, but ${other} found")
      case None ⇒
        Option(new BlowfishAdapter.BlowfishComposite(parent, SWT.NONE, None))
    }
  ...
  def dialog(parent: Shell, default: Option[Encryption.Parameters], tag: String = Messages.encryption_text) =
    default match {
      case Some(p: Blowfish.Parameters) ⇒
        Option(new BlowfishAdapter.BlowfishDialog(parent, Option(p), tag))
      case Some(other) ⇒
        throw new IllegalArgumentException(s"Expect Blowfish.Parameters, but ${other} found")
      case None ⇒
        Option(new BlowfishAdapter.BlowfishDialog(parent, None, tag))
    }
  /** Flag indicating whether the parameters are supported. */
  def parameters: Boolean = true
}

object BlowfishAdapter {
  /** Length parameters. */
  val length = Seq(("256", "build encryption key with length 256 bits long", Blowfish.Strength256),
    ("448", "build encryption key with length 448 bits long", Blowfish.Strength448))

  /**
   * Blowfish adapter composite
   */
  class BlowfishComposite(parent: Composite, style: Int, defaultParameters: Option[Blowfish.Parameters])
    extends BlowfishAdapterSkel(parent, style) with EncryptionAdapter.Composite {
    /** Binding context. */
    ...
    /** Key length field. */
    lazy val keyLengthField = WritableValue[(String, String, Blowfish.LengthParameter)]
    /** Secret field. */
    ...
    protected def initializeUI() {
      ...
      BlowfishAdapter.length.foreach(comboViewerKeyLength.add)
    ...
    protected def initializeDefaults() = defaultParameters match {
      case Some(parameters) ⇒
        BlowfishAdapter.length.find(_._3 == parameters.keyLength).foreach(value ⇒ getComboViewerKeyLength.setSelection(new StructuredSelection(value)))
    ...
    protected def updateResult = {
      ...
      case Some((length, secret, iv)) if secret.nonEmpty && ivFlagField.value && iv.nonEmpty ⇒
        result = Some(Right(Blowfish(secret, length, iv.getBytes(io.Codec.UTF8.charSet))))
      case Some((length, secret, _)) if secret.nonEmpty && !ivFlagField.value ⇒
        result = Some(Right(Blowfish(secret, length)))
      case _ ⇒
        result = Some(Left(Messages.parametersRequired_text))
    }
  }
  /**
   * Blowfish adapter dialog
   */
  class BlowfishDialog(parentShell: Shell, defaultValue: Option[Blowfish.Parameters], tag: String)
    extends TitleAreaDialog(parentShell) with EncryptionAdapter.Dialog {
    /** Private field with content's composite. */
    @volatile protected var content: BlowfishComposite = null
    ...
    override protected def configureShell(shell: Shell) {
      ...
      shell.setText(Messages.Adapter_selectXParameters_text.format(Blowfish.Identifier.name.capitalize, tag))
    }
    override def create() {
      super.create()
      setTitle(Blowfish.Identifier.name.capitalize)
      setMessage(Blowfish.Identifier.description.capitalize)
    }
    ...
    override protected def createDialogArea(parent: Composite): Control = {
      content = new BlowfishAdapter.BlowfishComposite(parent, SWT.NONE, defaultValue)
