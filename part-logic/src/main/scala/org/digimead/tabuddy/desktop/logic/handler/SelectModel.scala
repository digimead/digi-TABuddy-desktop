...
class SelectModel extends WorkbenchWindowControlContribution(SelectModel.id) with Loggable {
  ...
  protected def createCombo(parent: Composite): ComboViewer = {
    ...
    viewer.setLabelProvider(new SelectModel.ComboLabelProvider())
...
object SelectModel {
  ...
  private val instance = new mutable.WeakHashMap[SelectModel, Unit] with mutable.SynchronizedMap[SelectModel, Unit]
