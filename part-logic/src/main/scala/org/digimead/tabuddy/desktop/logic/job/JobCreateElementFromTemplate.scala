...
object JobCreateElementFromTemplate extends Loggable {
  @log
  def apply(template: ElementTemplate, container: Element.Generic): Option[Abstract] = {
    ...
    DI.jobFactory.asInstanceOf[Option[(ElementTemplate, Element[_ <: Stash], Symbol) => Abstract]] match {
      ...
      case None =>
        log.error("JobCreateElementFromTemplate implementation is not defined.")
        None
    }
  }

  abstract class Abstract(val template: ElementTemplate, val container: Element.Generic, val modelID: Symbol)
    extends Job[Element.Generic](s"Create a new element from $template for $container") with api.JobCreateElementFromTemplate
  /**
   * Dependency injection routines.
   */
  private object DI extends DependencyInjection.PersistentInjectable {
    // Element[_ <: Stash] == Element.Generic, avoid 'erroneous or inaccessible type' error
    lazy val jobFactory = injectOptional[(ElementTemplate, Element[_ <: Stash], Symbol) => api.JobCreateElementFromTemplate]
  }
}
