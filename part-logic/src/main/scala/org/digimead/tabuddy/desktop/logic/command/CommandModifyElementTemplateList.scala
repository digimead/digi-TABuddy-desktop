...
import org.digimead.tabuddy.desktop.core.definition.command.Command.parser.{ commandLiteral, commandRegex, sp }
...
object CommandModifyElementTemplateList extends Loggable {
  ...
  /** Command description. */
  implicit lazy val descriptor = Command.Descriptor(UUID.randomUUID())(Messages.modifyElementTemplateList_text,
    Messages.modifyElementTemplateListDescriptionShort_text, Messages.modifyElementTemplateListDescriptionLong_text,
  ...
  object HintContainer extends Command.Hint.Container {
    ...
    def apply(arg: String): Seq[Command.Hint] =
      (Core.context.context +: App.contextChildren(Core.context)).map {
        ...
        case Some(name) ⇒
          Some(Command.Hint(name, Some(s"Modify element template list for " +
