...
class Table(view: TableView, style: Int) extends Loggable {
  ...
  protected def createMenu(manager: IMenuManager, selection: IStructuredSelection) {
    ...
    // main menu
    manager.add(context.ActionConfigureColumns)
...
class TablePerShellContext(val area: WeakReference[TableView]) extends Table.PerShellContext
...
object Table extends ShellContext[TableView, TablePerShellContext] with Loggable {
  ...
  sealed trait PerShellContext extends ShellContext.PerShellContext[TableView] {
    ...
    object ActionConfigureColumns extends Action("Configure Columns...") {
      def apply() = area.get.foreach { area =>
        ConfigureColumns.forTable(area.table.tableViewer.getTable(), new SameShellProvider(area.getShell()))
      }
    ...
    object ActionAutoResize extends Action(Messages.autoresize_key, IAction.AS_CHECK_BOX) {
      ...
      def apply(immediately: Boolean = false) = area.get.foreach(area => if (immediately)
        area.table.autoresize(true)
      else
        future { area.table.autoresize(false) } onFailure {
    ...
    object ActionResetSorting extends Action(Messages.resetSorting_text) {
      ...
      def apply(immediately: Boolean = false) = area.get.foreach { area =>
        val comparator = area.table.tableViewer.getComparator().asInstanceOf[TableComparator]
        comparator.column = -1
        area.table.tableViewer.refresh()
      }
      override def run = apply()
    }
    class ActionSelectInTree(val element: Element.Generic) extends Action(Messages.select_text) {
      def apply() = area.get.foreach(a => a.tree.treeViewer.setSelection(new StructuredSelection(TreeProxy.Item(element)), true))
      override def run() = apply()
    }
    object ActionShowTree extends Action(Messages.tree_text) {
      def apply() = area.get.foreach { a =>
        a.context.ActionHideTree.setChecked(false)
