...
class Tree(view: TableView, style: Int) extends Loggable {
  ...
  /** Per shell context */
  protected[table] val context = Tree.perShellContextInitialize(view)
  ...
  /** The tree viewer */
  protected[table] val treeViewer = create()
  ...
  protected def autoresize(immediately: Boolean) = if (onActiveCalled.get)
    if (immediately)
      ...
      Thread.sleep(50)
      Main.execNGet { if (!treeViewer.getTree.isDisposed()) autoresizeUpdateControls() }
    } finally {
...
class TreePerShellContext(val area: WeakReference[TableView]) extends Tree.PerShellContext

object Tree extends ShellContext[TableView, TreePerShellContext] with Loggable {
  ...
  /** Create the new instance of PerShellContext */
  protected def perShellContextNewInstance(dialogOrViewOrOtherControl: WeakReference[TableView]): TreePerShellContext =
    new TreePerShellContext(dialogOrViewOrOtherControl)
  ...
  sealed trait PerShellContext extends ShellContext.PerShellContext[TableView] {
    ...
    protected[table] val expandedItems = mutable.HashSet[TreeProxy.Item]()

    /*
     * Actions
     */
    class ActionAsRoot(val element: Element.Generic) extends Action(Messages.markAsRoot_text) {
      def apply() = area.get.foreach(area => TableView.withRedrawDelayed(area) {
        area.tree.treeViewer.setInput(TreeProxy.Item(element))
        area.context.ActionAutoResize(false)
      })
    ...
    object ActionAutoResize extends Action(Messages.autoresize_key, IAction.AS_CHECK_BOX) {
      ...
      def apply(immediately: Boolean = false) = area.get.foreach(area => if (immediately)
        area.tree.autoresize(true)
      else
        future { area.tree.autoresize(false) } onFailure {
    ...
    class ActionCollapse(val element: Element.Generic) extends Action(Messages.collapseRecursively_text) {
      def apply() = area.get.foreach(area => TableView.withRedrawDelayed(area) {
        Tree.collapse(element, true, area.getShell)
        area.context.ActionAutoResize(false)
      })
      override def run() = apply()
    }
    class ActionExpand(val element: Element.Generic) extends Action(Messages.expandRecursively_text) {
      def apply() = area.get.foreach(area => TableView.withRedrawDelayed(area) {
        Tree.expand(element, true, area.getShell)
        area.context.ActionAutoResize(true)
      })
      override def run() = apply()
    }
    object ActionHideTree extends Action(Messages.hide_text) {
      def apply() = area.get.foreach { a =>
        a.context.ActionHideTree.setChecked(true)
        a.context.ActionHideTree()
      }
      override def run() = apply()
    }
    class ActionSelectInTable(val element: Element.Generic) extends Action(Messages.select_text) {
      def apply() = area.get.foreach(a => a.table.tableViewer.setSelection(new StructuredSelection(TreeProxy.Item(element)), true))
      override def run() = apply()
    }
  }
}
