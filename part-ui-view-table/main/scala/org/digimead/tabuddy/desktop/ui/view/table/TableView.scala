...
class TableViewPerShellContext(val area: WeakReference[TableView]) extends TableView.PerShellContext
...
object TableView extends ShellContext[TableView, TableViewPerShellContext] with Main.Interface with Loggable {
  ...
  sealed trait PerShellContext extends ShellContext.PerShellContext[TableView] {
    lazy val toolbarView = new ToolbarView(area)
    lazy val toolbarPrimary = new ToolbarPrimary(area)
    lazy val toolbarSecondary = new ToolbarSecondary(area)
    ...
    object ActionAutoResize extends Action(Messages.autoresize_key) {
      ...
      def apply(immediately: Boolean) = area.get.foreach { area =>
        if (area.tree.context.ActionAutoResize.isChecked())
          area.tree.context.ActionAutoResize(immediately)
        if (area.table.context.ActionAutoResize.isChecked())
          area.table.context.ActionAutoResize(immediately)
      }
    }
    ...
    object ActionHideTree extends Action(Messages.autoresize_key, IAction.AS_CHECK_BOX) {
      ...
      def apply() = area.get.foreach(area => if (isChecked())
        area.getSashForm.setMaximizedControl(area.table.tableViewer.getTable())
      else
        area.getSashForm.setMaximizedControl(null))
