...
import org.digimead.tabuddy.desktop.ui.dialog.ElementCreate
...
class JobShowElementCreate private (val container: Element.Generic, val modelID: Symbol)
...
object JobShowElementCreate {
  def apply(container: Element.Generic): Option[JobBuilder[JobShowElementCreate]] = {
    val modelID = Model.eId
    Some(new JobBuilder(JobShowElementCreate, () => new JobShowElementCreate(container, modelID)))
  }
}
