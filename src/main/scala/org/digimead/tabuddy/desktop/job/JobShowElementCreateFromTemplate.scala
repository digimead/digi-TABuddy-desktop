...
import org.digimead.tabuddy.desktop.ui.dialog.ElementEditor
...
class JobShowElementCreateFromTemplate private (val template: ElementTemplate.Interface, val container: Element.Generic, val modelID: Symbol)
...
object JobShowElementCreateFromTemplate {
  def apply(template: ElementTemplate.Interface, container: Element.Generic): Option[JobBuilder[JobShowElementCreateFromTemplate]] = {
    val modelID = Model.eId
    Some(new JobBuilder(JobShowElementCreateFromTemplate, () => new JobShowElementCreateFromTemplate(template, container, modelID)))
  }
}
