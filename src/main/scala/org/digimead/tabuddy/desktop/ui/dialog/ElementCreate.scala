...
import org.digimead.tabuddy.desktop.job.JobShowElementCreateFromTemplate
...
class ElementCreate(val parentShell: Shell, container: Element.Generic)
  extends org.digimead.tabuddy.desktop.res.dialog.ElementCreate(parentShell) with Dialog with Loggable {
  /** Close dialog on mouse over */
  val guargThread = new Thread(new Runnable {
    def run = {
      var run = !getShell.isDisposed()
      while (run) {
        val bounds = Main.execNGet {
          val shell = getShell
  ...
  /** Available templates */
  val templateList = Data.elementTemplates.toList.filter(_.availability).sortBy(_.id.name)
  ...
  override protected def createDialogArea(parent: Composite): Control = {
    ...
    templates.setBackground(SWTResourceManager.getColor(SWT.COLOR_WHITE))
    ...
    Main.display.getMonitors().find(_.getBounds.contains(cursorPoint)) match {
...
object ElementCreate {
  ...
  class ButtonListener(dialog: ElementCreate, template: ElementTemplate.Interface, container: Element.Generic) extends Listener {
    def handleEvent(event: Event) = {
      dialog.close()
      JobShowElementCreateFromTemplate(template, container).foreach(_.execute)
    }
  }
}
