...
import org.digimead.tabuddy.desktop.job.JobShowTypeList
import org.digimead.tabuddy.desktop.payload.Payload
import org.digimead.tabuddy.desktop.res.Messages
import org.eclipse.jface.action.Action

object ActionShowTypeList extends Action(Messages.types_text) {
  ...
  override def run = JobShowTypeList(Data.typeSchemas.toSet, Data.typeSchema.value).foreach(_.setOnSucceeded { job =>
    job.getValue.foreach {
      case (newSchemaSet, newActiveSchema) => Main.exec {
        Data.typeSchemas.clear
        Data.typeSchemas ++= newSchemaSet
        Data.typeSchema.value = newActiveSchema
      }
    }
  }.execute)
}
