...
class TranslationLookup(val parentShell: Shell)
  ...
  protected def autoresize() = if (autoResizeLock.tryLock()) try {
    ...
    Main.execNGet {
      if (!getTableViewer.getTable.isDisposed()) {
        adjustColumnWidth(getTableViewerColumnKey(), Dialog.columnPadding)
  ...
  override protected def createDialogArea(parent: Composite): Control = {
    ...
    val filterListener = new IChangeListener() {
...
object TranslationLookup extends Loggable {
  ...
  class TranslationComparator extends ViewerComparator {
    private var _column = TranslationLookup.sortColumn
    private var _direction = TranslationLookup.sortDirection
    ...
    def column_=(arg: Int) {
      _column = arg
      TranslationLookup.sortColumn = _column
      _direction = TranslationLookup.defaultDirection
      TranslationLookup.sortDirection = _direction
    }
    /** Sorting direction */
