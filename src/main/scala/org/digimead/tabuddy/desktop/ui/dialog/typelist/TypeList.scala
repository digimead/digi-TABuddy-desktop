...
import org.digimead.tabuddy.desktop.job.JobShowTypeEditor
...
class TypeList(val parentShell: Shell, val initial: List[TypeSchema.Interface], val initialActiveSchema: TypeSchema.Interface)
  extends org.digimead.tabuddy.desktop.res.dialog.TypeList(parentShell) with Dialog with Loggable {
  ...
  protected val schemaField = WritableValue[TypeSchema.Interface]
  ...
  def autoresize() = if (autoResizeLock.tryLock()) try {
    Thread.sleep(50)
    Main.execNGet {
      if (!getTableTypeSchemas.getTable.isDisposed()) {
        adjustColumnWidth(getTableViewerColumnName, Dialog.columnPadding)
        getTableTypeSchemas.refresh()
      }
    }
  } finally {
  ...
  def getActiveSchema(): TypeSchema.Interface = actual.find(_.id == actualActiveSchema.value).getOrElse(initialActiveSchema)
  /** Get modified type schemas */
  def getSchemaSet(): Set[TypeSchema.Interface] = actual.toSet
  /** Create contents of the dialog. */
  override protected def createDialogArea(parent: Composite): Control = {
    val result = super.createDialogArea(parent)
    new ActionContributionItem(TypeList.ActionActivate).fill(getCompositeActivator())
    new ActionContributionItem(TypeList.ActionCreate).fill(getCompositeTypes())
    new ActionContributionItem(TypeList.ActionCreateFrom).fill(getCompositeTypes())
    new ActionContributionItem(TypeList.ActionEdit).fill(getCompositeTypes())
    new ActionContributionItem(TypeList.ActionRemove).fill(getCompositeTypes())
    new ActionContributionItem(TypeList.ActionReset).fill(getCompositeTypes())
    ...
    val actualActiveSchemaListener = actualActiveSchema.addChangeListener { event =>
      ...
      getTableTypeSchemas.getSelection match {
  ...
  protected def initTableTypeSchemas() {
    val viewer = getTableTypeSchemas()
    ...
    viewer.addSelectionChangedListener(new ISelectionChangedListener() {
      override def selectionChanged(event: SelectionChangedEvent) = event.getSelection() match {
        case selection: IStructuredSelection if !selection.isEmpty() =>
          val selected = selection.getFirstElement().asInstanceOf[TypeSchema.Interface]
    ...
    Main.bindingContext.bindValue(ViewersObservables.observeSingleSelection(viewer), schemaField)
  }
  /** On dialog active */
  ...
  protected[typelist] def updateActualSchema(before: TypeSchema.Interface, after: TypeSchema.Interface) {
    ...
    if (index == actual.size - 1)
      getTableTypeSchemas.refresh() // Workaround for the JFace bug. Force the last element modification.
    getTableTypeSchemas.setSelection(new StructuredSelection(after), true)
  }
...
object TypeList extends Loggable {
  ...
  def schema[T](f: (TypeList, TypeSchema.Interface) => T): Option[T] =
  ...
  object ActionCreate extends Action(Messages.create_text) with Loggable {
    override def run = TypeList.dialog.foreach { dialog =>
      ...
      JobShowTypeEditor(newSchema, dialog.actual.toList, newSchema == dialog.actualActiveSchema.value).
  ...
  object ActionEdit extends Action(Messages.edit_text) {
    override def run = TypeList.schema { (dialog, before) =>
      JobShowTypeEditor(before, dialog.actual.toList, before == dialog.actualActiveSchema.value).
        foreach(_.setOnSucceeded { job =>
          job.getValue.foreach {
