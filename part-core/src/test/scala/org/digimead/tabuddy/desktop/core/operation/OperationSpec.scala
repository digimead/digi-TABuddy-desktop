...
class OperationSpec extends FeatureSpec with GivenWhenThen with Test.Base {
  feature("OperationCommands") {
    scenario("invoke directly before initialization") {
      OperationCommands.operation(true) should be(Seq.empty)
      OperationCommands.operation(false) should be(Seq.empty)
    }
    scenario("invoke directly after initialization") {
      Commands.configure()
      OperationCommands.operation(true) should have size (3)
      OperationCommands.operation(false) should have size (4)
      val ctxChild = Core.context.createChild()
      ctxChild.activate()
      OperationCommands.operation(true) should have size (0)
      OperationCommands.operation(false) should have size (4)
      ctxChild.deactivate()
      OperationCommands.operation(true) should have size (3)
      OperationCommands.operation(false) should have size (4)
    ...
    scenario("invoke operation") {
      ...
      OperationCommands(true).foreach { operation ⇒
