...
/** Wrapper for menu item that is based on IAction. */
class JFaceAItem(
  /** Original menu element. */
  override val element: ActionContributionItem) extends Item[ActionContributionItem](element) with Loggable {
  /** Contains uniform menu element. */
  lazy val uniform: JFaceAItem.Uniform = JFaceAItem.Uniform(this)
...
object JFaceAItem {
  case class Uniform(
    /** Item wrapper. */
    val wrapper: JFaceAItem) extends Item.Uniform[ActionContributionItem] {
