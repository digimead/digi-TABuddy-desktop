...
object TypeSchema extends DependencyInjection.PersistentInjectable with Loggable {
  ...
  def load(): Set[TypeSchema.Interface] = {
  ...
  def save(schemas: Set[TypeSchema.Interface]) = Main.exec {
  ...
  object YAML extends Payload.YAMLProcessor[TypeSchema.Interface] {
    ...
    def from(data: String): Option[TypeSchema.Interface] = {
      val yaml = new Yaml(new TypeSchemaConstructor)
      Option(yaml.load(data).asInstanceOf[TypeSchema.Interface])
    }
