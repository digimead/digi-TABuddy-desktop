...
abstract class JobModifyFilter(val filter: Filter, val filterList: Set[Filter], val modelID: Symbol)
  extends Job[Filter]("Edit filter %s for model %s".format(filter, Model.eId))
