...
package org.digimead.tabuddy.desktop.payload.view
...
case class View(
  ...
  def canEqual(other: Any) =
    other.isInstanceOf[org.digimead.tabuddy.desktop.payload.view.View]
  override def equals(other: Any) = other match {
    case that: org.digimead.tabuddy.desktop.payload.view.View =>
      (this eq that) || {
        that.canEqual(this) &&
...
object View extends DependencyInjection.PersistentInjectable with Loggable {
  ...
  def container(): Element.Generic = inject[Record.Interface[_ <: Record.Stash]]("eViewDefinition")
  ...
  def save(views: Set[View]) = Main.exec {
