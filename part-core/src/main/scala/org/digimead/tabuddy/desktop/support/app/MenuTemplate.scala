...
abstract class MenuTemplate extends menu.Template[MenuTemplate] {
  /**
   ...
   */
  lazy val uniform: MenuTemplate.Uniform = MenuTemplate.Uniform(this)
  /**
  ...
  override def contribution: MenuManager

  override def toString() = s"MTemplate[${getClass.getName().split("""\.""").last}/${label}]"
}

object MenuTemplate {
  case class Uniform(
    /** Menu wrapper. */
    val wrapper: MenuTemplate) extends menu.Template.Uniform[MenuTemplate] {
  }
  object Uniform {
