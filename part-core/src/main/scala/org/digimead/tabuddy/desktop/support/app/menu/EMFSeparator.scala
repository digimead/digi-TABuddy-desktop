...
class EMFSeparator(
  ...
  override val element: MMenuSeparator with Container) extends Item[MMenuSeparator with Container](element) with EMFBase[MMenuSeparator with Container] with Loggable {
  /** Contains uniform menu element. */
  lazy val uniform: EMFSeparator.Uniform = EMFSeparator.Uniform(this)

  /** Adjust exists entity with template values */
  @log
  def adjust[T](template: MenuItemTemplate)(f: Item[_] => T) {
    log.debug(s"Adjust EMF separator '${uniform.label}' from " + template)
  }
}
...
object EMFSeparator {
  case class Uniform(
    /** Item wrapper. */
    val wrapper: EMFSeparator) extends Item.Uniform[MMenuSeparator with Container] {
     ...
     */
    def enabled: Option[Boolean] = Option(true)
    /**
