...
case class Sorting(
  ...
  def canEqual(other: Any) =
    other.isInstanceOf[org.digimead.tabuddy.desktop.payload.view.Sorting]
  override def equals(other: Any) = other match {
    case that: org.digimead.tabuddy.desktop.payload.view.Sorting =>
      (this eq that) || {
        that.canEqual(this) &&
...
object Sorting extends DependencyInjection.PersistentInjectable with Loggable {
  ...
  def container(): Element.Generic = inject[Record.Interface[_ <: Record.Stash]]("eViewSorting")
  ...
  def save(sortings: Set[Sorting]) = Main.exec {
