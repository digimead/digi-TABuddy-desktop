...
case class Filter(
  ...
  def canEqual(other: Any) =
    other.isInstanceOf[org.digimead.tabuddy.desktop.payload.view.Filter]
  override def equals(other: Any) = other match {
    case that: org.digimead.tabuddy.desktop.payload.view.Filter =>
      (this eq that) || {
        that.canEqual(this) &&
...
object Filter extends DependencyInjection.PersistentInjectable with Loggable {
  ...
  def container(): Element.Generic = inject[Record.Interface[_ <: Record.Stash]]("eViewFilter")
    ...
    val rulesRaw = for (i <- 0 until Filter.collectionMaximum if next) yield {
      element.eGet[String](getFieldIDRuleProperty(i)) match {
        case Some(property) =>
          ...
          } yield Rule(Symbol(property), Symbol(propertyType), not, filter, argument)
        case None =>
          next = false
          None
      }
  ...
  def save(filters: Set[Filter]) = Main.exec {
