...
trait ConditionMap[T] {
  ...
  /** Invoke user test. */
  def test(): Unit = testLock.synchronized { test(conditions.toMap[String, T]) }
  /** Set new value. */
  ...
  /** User test that is based on condition map. */
  protected def test(map: immutable.Map[String, T]): Unit
}
