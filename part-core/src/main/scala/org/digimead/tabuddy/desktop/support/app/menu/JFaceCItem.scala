...
class JFaceCItem(
  ...
  override val element: IContributionItem) extends Item[IContributionItem](element) with Loggable {
  /** Contains uniform menu element. */
  lazy val uniform: JFaceCItem.Uniform = JFaceCItem.Uniform(this)
...
object JFaceCItem {
  case class Uniform(
    /** Item wrapper. */
    val wrapper: JFaceCItem) extends Item.Uniform[IContributionItem] {
