 ...
 */
object CommandExit extends XLoggable {
  import Command.parser._
  /** Akka execution context. */
  implicit lazy val ec = App.system.dispatcher
  /** Command description. */
  implicit lazy val descriptor = Command.Descriptor(UUID.randomUUID())(Messages.exit_text,
    Messages.exitDescriptionShort_text, Messages.exitDescriptionLong_text,
    (activeContext, parserContext, parserResult) ⇒ Future {
      val exchanger = new Exchanger[Operation.Result[Unit]]()
      OperationExit().foreach { operation ⇒
