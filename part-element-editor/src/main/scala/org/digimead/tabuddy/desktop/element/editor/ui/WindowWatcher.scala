...
package org.digimead.tabuddy.desktop.element.editor.ui
...
class WindowWatcher extends Actor with XLoggable {
  ...
  protected def adjustToolbar(window: AppWindow) {
    val elementToolbar = SmartToolbarManager(window, WindowWatcher.elementToolbar)
    SmartToolbarManager.add(elementToolbar, ContextInjectionFactory.make(classOf[action.ActionCreateElement], window.windowContext))
    SmartToolbarManager.add(elementToolbar, ContextInjectionFactory.make(classOf[action.ActionModifyElement], window.windowContext))
    SmartToolbarManager.add(elementToolbar, ContextInjectionFactory.make(classOf[action.ActionDeleteElement], window.windowContext))
  }

  override def toString = "element.editor.ui.WindowWatcher"
...
object WindowWatcher extends XLoggable {
  ...
  /** Element editor toolbar descriptor. */
  lazy val elementToolbar = App.execNGet { SmartToolbarManager.Descriptor(getClass.getName() + "#element") }

  /** WindowWatcher actor reference configuration object. */
  def props = DI.props

  override def toString = "element.editor.ui.WindowWatcher[Singleton]"
  ...
  private object DI extends XDependencyInjection.PersistentInjectable {
    /** WindowWatcher actor reference configuration object. */
    lazy val props = injectOptional[Props]("ElementEditor.WindowWatcher") getOrElse Props[WindowWatcher]
  }
}
